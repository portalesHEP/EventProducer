...
#COMMON_PARAM="--FCCee --STDHEP --send --typestdhep wzp6 -n 100000 --prodtag ${PROD_TAG} --condor -q longlunch --priority group_u_CMST3.all"
COMMON_PARAM="--FCCee --STDHEP --send --typestdhep wzp6 -n 100000 --prodtag ${PROD_TAG} --condor -q longlunch --priority group_u_CMS.u_zh.users"

# Njob per process = requested_stat / Nevt_per_job (= requested_stat / 100000)
...
do
    ngoodsample=`ls -l /eos/experiment/fcc/ee/generation/stdhep/winter2023/${card}/ | grep events | wc -l`
    ngood=$(( $ngood + $ngoodsample ))
    ntot=$(( $ntot + ${NJOB_PROCESS[$card]} ))
    if [[ "$ngoodsample" != "${NJOB_PROCESS[$card]}" ]]
    then
        echo ${card} :$ngoodsample / ${NJOB_PROCESS[$card]}
    fi
    
done

echo TOTAL: $ngood / $ntot
